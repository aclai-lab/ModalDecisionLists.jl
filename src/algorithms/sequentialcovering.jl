...
function IREP_Star(
    ...
    while length(rulebase) < max_rulebase_length
        ...
        # indici in uncoveredX che sono stati coperti dalla regola
        ...
        #println("bestantecedent_prune_cov: \n$bestantecedent_prune_cov")
...
"""
function compute_coverage(antecedent::LeftmostConjunctiveForm, 
    growX, grow_inds, 
    pruneX, prune_inds, 
    bestantecedent_prune_cov
)    
...
# ritorna un'approssimazione di log2(n!) usando Stirling
function log2_factorial(n::Int)::Real
    ...
    # piuttosto buona per n piccolo, in questa maniera non serve realmente fare un if n < n_min per usare il fattoriale
    # su piccoli valori
...
function rs_dataset_bits(
    ...
    # TODO: Posso pensare di riciclare questa sat_mask tra un'iterazione e l'altra. Ad ogni iterazione del ciclo soprastante
    # Sto aggiungendo condizioni, ma se ci prima dell'inserimento di una nuova regola ci sono N regola R1, ..., RN, queste ci 
    # saranno anche dopo, ergo sto ricalcolando la tessa cosa ad ogni iterazione del loop "while true" principale di IREP*
    # posso risparmiarmi molte check
    ...
    #end
    rule_sat_mask = check(rule.antecedent, X)
    ruleset_sat_mask = prev_ruleset_satmask .| rule_sat_mask
